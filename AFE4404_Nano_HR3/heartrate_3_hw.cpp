...
{
    Serial.println("hr3 init");    
    hr3_hal_init( address );
    hr3_set_settings( sw_reset_en,diag_mode_dis,susp_count_dis,reg_read_dis );



    hr3_set_led2_start_end( 0, 398 );                     // Turn on LED 2
    hr3_set_led2_sample_start_end( 100, 398 );            // Sampling  
    hr3_set_adc_reset0_start_end( 5600, 5606 );           // Reset ADC
    hr3_set_led2_convert_start_end( 5608, 6067 );         // Convert
    hr3_set_led3_start_stop( 400, 798 );                  // Turn on LED 3
    hr3_set_led3_sample_start_end( 500, 798 );            //....
    hr3_set_adc_reset1_start_end( 6069, 6075 );
    hr3_set_led3_convert_start_end( 6077, 6536 );    
    hr3_set_led1_start_end( 800, 1198 );                  // Turn on LED 1
    hr3_set_led1_sample_start_end( 900, 1198 );           //...
    hr3_set_adc_reset2_start_end( 6538, 6544 );
    hr3_set_led1_convert_start_end( 6546, 7006 );
    hr3_set_amb1_sample_start_end( 1300, 1598 );          //Sampling ambient value
    hr3_set_adc_reset3_start_end( 7008, 7014 );           // Reset 
    hr3_set_amb1_convert_start_end( 7016, 7475 );         // Convert
    hr3_set_prgrmmbl_timing_start_end( 7477, 7577);             //lengthen interrupt
...
//    hr3_set_tia_gain( false, 0, 3 );
    hr3_set_tia_gain( true, 0, 3 );    //  lengthen interrupt
    
//    hr3_set_led_currents( 15,3,3 ); //LED 1: Green , LED 2: RED, LED 3: IR
    hr3_set_led_currents( 0,3,3 );
//    hr3_set_led_currents( 0,10,10 );
